...
class PrometheusConsumer(BaseConsumer):
    ...
    def process_message(self, message):
        ...
        if 'avg_value' in data:
            ...
            print(f"📊 [Spark] {sensor} Aggregation: Avg={data['avg_value']:.2f}", flush=True)
    ...
    def check_anomalies(self, sensor, data):
        ...
        # Rule 1: High Pressure (PS1 > 300 bar) - Use max to catch spikes
        if sensor == 'PS1' and max_val > 300:
            anomalies.append('high_pressure')
        
        # Rule 2: High Temperature (TS1 > 100 C)
        if sensor == 'TS1' and max_val > 100:
