...
def main():
    ...
    MLFLOW_TRACKING_URI = os.getenv("MLFLOW_TRACKING_URI", "http://mlflow-server.monitoring.svc.cluster.local:5000")

    # 1. Load Data
    df_sensors = spark.read.parquet(f"{HDFS_NAMENODE}/hydraulic/raw")
    df_labels = spark.read.parquet(f"{HDFS_NAMENODE}/hydraulic/labels")
