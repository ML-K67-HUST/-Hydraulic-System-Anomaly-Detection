...
# Label configuration: (filename, topic_name)
LABEL_CONFIG = ("profile.txt", "hydraulic-labels")
...
class HydraulicSystemProducer:
    ...
    def __init__(self, cycle_idx=0):
        ...
        # Add label producer (one per cycle)
        self.label_message = self.create_label_message()
    
    def create_label_message(self):
        """Load labels for the current cycle and format into a message"""
        try:
            filename, _ = LABEL_CONFIG
            data = load_cycle_data(filename, self.cycle_idx)
            # data has 5 labels: [Cooler, Valve, Pump, Accumulator, Stable]
            return {
                "cycle": self.cycle_idx,
                "label_cooler": int(data[0]),
                "label_valve": int(data[1]),
                "label_pump": int(data[2]),
                "label_accumulator": int(data[3]),
                "label_stable": int(data[4]),
                "timestamp": time.time()
            }
        except Exception as e:
    ...
    def send_labels(self):
        """Send the label message to Kafka"""
        if not self.label_message:
            return
            
        _, topic = LABEL_CONFIG
        try:
            # Use one of the per-thread producers or create a temporary one
            # For simplicity, creating a temporary one
            producer = KafkaProducer(
                bootstrap_servers=KAFKA_BROKER,
                value_serializer=lambda v: json.dumps(v).encode('utf-8')
            )
            producer.send(topic, value=self.label_message)
            producer.flush()
            producer.close()
            print(f"[LABELS] Sent labels for cycle {self.cycle_idx}")
        except Exception as e:
            print(f"[LABELS] Error sending labels: {e}")
    ...
    def start(self):
        ...
        # Send labels at the end of the cycle (or beginning, but end is fine)
        self.send_labels()
...
def main():
    ...
    if sys.argv[1] == "--all":
        ...
        # Run n cycles continuously (infinite if n is not specified or -1)
        n = -1  # default to infinite
        if len(sys.argv) > 2:
            try:
                n = int(sys.argv[2])
            except ValueError:
        ...
        if n > 0:
            ...
            print(f"🔄 Running continuously (infinite mode)...")
        print()
        ...
        while n == -1 or i < n:
            ...
            if n > 0:
                ...
                print(f"📊 Iteration {i + 1} - Cycle {cycle_idx}")
            print(f"{'='*80}")
