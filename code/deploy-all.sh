...
# Check and fix inotify limits (Important for large Kind clusters - Linux Only)
if [ "$(uname)" == "Linux" ]; then
...
if [[ "$(docker images -q hadoop-python:3.8 2> /dev/null)" == "" ]]; then
    ...
    print_info "Image hadoop-python:3.8 exists, skipping build."
fi

if [[ "$(docker images -q spark-client:custom 2> /dev/null)" == "" ]]; then
    ...
    print_info "Image spark-client:custom exists, skipping build."
fi
...
#     # check_pod_status "hdfs"
#     # exit 1
#     print_warn "Format job failed/timed out, but NameNode execution continues (assuming auto-format worked)"
# fi
...
print_info "Creating Spark App Code ConfigMap..."
kubectl create configmap spark-app-code --from-file=spark-apps/ -n hdfs --dry-run=client -o yaml | kubectl apply -f -
